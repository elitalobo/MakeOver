...
describe('Image Search', function() {
  this.timeout(30000);
  describe('#Invalid Keywords', function() {
    this.timeout(30000);
    it('Expected Invalid Keywords message', function(done) {
		socket.emit('getImages',{ keywords: "xyz123"} );
		socket.on('got_error', function(data) {
        			assert.deepEqual(data.message, "No images were returned by sources! Check keywords!") 
        			done();
        		});

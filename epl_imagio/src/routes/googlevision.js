...
GoogleVision.prototype.getDetails = function(photos, file_paths, uid, old_callback) {
    var details = {}
    var queue = async.queue(function(task, callback) {
                task.func(task.uid, callback);
	},100);
    ...
    queue.push({ uid: uid, func: function (uid, callback) {
	...
	if(apiResponse) {
    	...
    	if (err || apiResponse==null || apiResponse==undefined  ) {
		 logger.log('error',"Request Id: " + uid + " Got error while detecting image properties. ",err);
		 callback(err, null);
	
    	} else {
     				
			logger.log('info', "Request Id: " + uid + " Got image properties from Google Vision ", apiResponse);
	...
	queue.push({ uid: uid,func: function(uid, callback) {
		...
		if(err) {
			logger.log('error', "Request Id: " + uid + " Got image properties from Google Vision " , apiResponse);
			callback(err, null);
		
		} else {
			logger.log('info', "Request Id: " + uid + " Got image labels from Google Vision ", apiResponse);
...
GoogleVision.prototype.processImages = function(photos,uid,callback) {
	var urls = []
	console.log("Called processImages ");
	for(var cnt=0;cnt<photos.length;cnt++){
		urls.push(photos[cnt].photo.urls[0].url);
		console.log(cnt + " " + photos[cnt].photo.urls[0].url);
	}
	console.log("done");
	this.getDetails(photos,urls,uid, callback);
}

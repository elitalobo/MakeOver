...
function recursiveFrameScan() {
    console.log(visiturl + "Recursive Framework");
...
casper.on('navigation.requested', function(url, type, willNavigate, main) {
    console.log(url);
    console.log('> Trying to navigate to: ' + url + ' (main frame: ' + main + ') ' + address);
...
setTimeout(function() {
	if(flag==false) {
		console.log("dying " + address);
		casper.echo("########################################### Exiting #######################################").die();
	}
}, 75000);
...
casper.start(address, function() {   
    ...
    setTimeout(function() {
	if(new_flag==false) {
		casper.echo("######################################################################## Exiting again ########################################").die();
		console.log("dying again ") + address;
	}
    },60000);
